  ...
  <div class="wrraper">
    ...
    <main>
      ...
      >
        ...
        <!-- ======================== Start Top Section ======================== -->
        <template v-slot:top>
          <h3 class="title table-title d-inline-flex">الشروط والأحكام</h3>
          <!-- Delete dialog -->
...
export default {
  ...
  methods: {
    ...
    // ==================== Start CRUD ====================
    addItem() {
      this.$router.push({ path: '/terms/add' })
    },
    showItem(item) {
      this.$router.push({ path: '/terms/show/' + item.id })
    },
    editItem(item) {
    ...
    deleteItemConfirm() {
      this.$axios({
        method: 'DELETE',
        url: `terms/${this.itemtoDelete.id}`,
      })
        .then(() => {
