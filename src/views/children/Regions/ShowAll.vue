<template>
  <div class="wrraper">
    ...
    <Breadcrumb
      :items="items"
      search_route="/regions/add"
      search_title="اضافه منطقة جديده"
      icon="fa-plus"
    />
    <!-- End Breadcrumb -->
...
export default {
  data() {
    return {
      ...
      items: [
        ...
        {
          text: this.$t('breadcrumb.regions.all'),
          disabled: true,
          href: '',
        },
      ],
  ...
  methods: {
    ...
    // ==================== Start CRUD ====================
    addItem() {
      this.$router.push({ path: '/regions/add' })
    },
    showItem(item) {
    ...
    deleteItemConfirm() {
      this.$axios({
        method: 'DELETE',
        url: `region/${this.itemtoDelete.id}`,
      })
        .then(() => {
