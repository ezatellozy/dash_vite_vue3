...
export default {
  ...
  data() {
    return {
      ...
      items: [
        ...
        {
          text: this.$t('breadcrumb.regions.title'),
          disabled: false,
          href: '/regions/show-all',
        },
        {
          text: this.$t('breadcrumb.regions.add'),
          disabled: true,
          href: '',
        },
      ],
  ...
  methods: {
    addDataLocalStorage() {
      localStorage.setItem('region_data', JSON.stringify(this.data))
    },

    getDataLocalStorage() {
      if (localStorage.getItem('region_data')) {
        this.data = JSON.parse(localStorage.getItem('region_data'))
      }
    },
    merkersChanged(data) {
      // console.log(data)
    ...
    submitData() {
      ...
      this.$axios({
        ...
        .then(() => {
          ...
          localStorage.removeItem('region_data')
          ...
          this.$router.push({ path: '/regions/show-all' })
          this.btnIsLoading = false
        })
        .catch((err) => {
